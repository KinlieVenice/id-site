...
const FEATURES = [
  'Official sizes for 100+ countries',
  'Background removal',
  'Retouch and enhance',
...
export default function PhotoStep({ onImage }) {
      ...
      <div className="two-col">
        <div>
          ...
          >
            ...
            <p className="hint">JPG, PNG · Max 20MB · Processed on your device</p>
            <input
              ref={inputRef}
              type="file"
        ...
        <div className="card side-card">
          ...
          <ul className="feature-list">
            {FEATURES.map((f) => (
              <li key={f}>
                <Icon name="check_circle" fill /> {f}
              </li>
            ))}
          </ul>

          <div className="example-label good">
            Good example <Icon name="check_circle" fill />
          </div>
          <div className="example-grid">
            {[0, 1, 2].map((i) => (
              <div className="example-thumb" key={i}>
                <Icon name="person" style={{ fontSize: 34, color: 'var(--success)', margin: 'auto' }} />
          ...
          <div className="example-grid">
            {[0, 1, 2].map((i) => (
              <div className="example-thumb" key={i}>
                <Icon name="person" style={{ fontSize: 34, color: 'var(--danger)', margin: 'auto' }} />
